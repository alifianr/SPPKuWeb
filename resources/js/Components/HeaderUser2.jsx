                ...
                <div ref={dropdownRef} className="relative mobile:w-24 lg:w-44">
                    ...
                    {isDropdownOpen && (
                        <div className="absolute left-0 mt-2 w-full bg-white rounded-lg shadow-lg z-50">
                            <Link href="/user/profile" className="flex items-center py-2 px-4 text-black hover:bg-yellow-500 hover:text-white mobile:text-[9px]">
                                <FontAwesomeIcon icon={faUser} className="mr-2 mobile:text-xs lg:text-lg" />
                                Profil Saya
                            </Link>
                            <button
                                onClick={handleLogout}
                                className="flex items-center w-full text-left px-4 py-2 text-black hover:bg-red-500 hover:text-white cursor-pointer mobile:text-[9px]"
                            >
                                <FontAwesomeIcon icon={faSignOutAlt} className="mr-2 mobile:text-xs lg:text-lg" />
                                Logout
                            </button>
                        </div>
                    )}
            ...
            {isMobileMenuOpen && (
                <div className="md:hidden fixed top-0 left-0 w-full h-full bg-white flex flex-col items-center justify-start z-50">
                    ...
                    <nav className="flex flex-col space-y-4 text-center w-full max-w-xs">
                        <Link
                            href="/user/homepage"
                            className={`block text-lg font-bold py-3 w-full rounded ${activeTab === "/user/homepage"
                                ? "bg-green-700 text-white"
                                : "border border-green-700 text-green-700 hover:bg-green-700 hover:text-white"
                                }`}
                            onClick={() => {
                                setActiveTab("/user/homepage");
                        ...
                        <Link
                            ...
                            className={`block text-lg font-bold py-3 w-full rounded ${["/user/pembayaran", "/user/proses-pembayaran"].includes(activeTab)
                                ? "bg-green-700 text-white"
                                : "border border-green-700 text-green-700 hover:bg-green-700 hover:text-white"
                                }`}
                            onClick={() => {
                                setActiveTab("/user/pembayaran");
                                setIsMobileMenuOpen(false);
                            }}
                        >
                            Pembayaran SPP
