...
export default function ButtonPayment2() {
    ...
    const downloadAlert = () => {
        Swal.fire({
            ...
            window.location.href = "/user/download-bill";
        });
    };
            ...
            <div className="flex flex-col mobile:flex-row sm:items-center sm:justify-between space-y-4 mobile:space-y-0">
                ...
                <div className="flex flex-col sm:flex-row items-center space-y-2 mobile:space-y-2 sm:space-x-4 w-full sm:w-auto">
                    <a
                        ...
                        className={`text-center py-2 font-bold w-full sm:w-auto mobile:rounded-lg mobile:text-[11px] mobile:px-2 lg:text-lg lg:px-6 lg:rounded-full ${activeTab === "/user/pembayaran"
                            ? "bg-green-700 text-white"
                            : "border border-green-700 text-green-700 hover:outline hover:outline-2 hover:outline-green-900"
                            }`}
                        onClick={() => setActiveTab("/user/pembayaran")}
                    >
                        <FontAwesomeIcon icon={faFileAlt} className="mr-2" /> Semua Pembayaran SPP
                    </a>
                    <a
                        href="/user/user-menunggu-pembayaran"
                        className={`text-center py-2 font-bold w-full sm:w-auto mobile:rounded-lg mobile:text-[11px] mobile:px-2 lg:text-lg lg:px-6 lg:rounded-full ${activeTab === "/user/user-menunggu-pembayaran"
                            ? "bg-green-700 text-white"
                            : "border border-green-700 text-green-700 hover:outline hover:outline-2 hover:outline-green-900"
                            }`}
                        onClick={() => setActiveTab("/user/user-menunggu-pembayaran")}
                    >
