            ...
            <div className="w-full mx-auto mt-6 rounded-lg lg:px-16">
                ...
                <div className="mb-6">
                    ...
                    <div className="flex gap-4 text-sm md:text-base">
                        ...
                        <div className="ml-16">
                            <p>: {studentData.student_nisn || "-"}</p>
                            <p>: {studentData.student_name || "-"}</p>
                            <p>: {studentData.student_class || "-"}</p>
                            <p>: {studentData.student_category || "-"}</p>
                        </div>
                    </div>
                </div>
                                ...
                                {payment.length > 0 ? (
                                    payment.map((pay, index) => (
                                        <tr key={index} className="hover:bg-gray-100">
                                            ...
                                            <td className="py-2 px-4 border text-center">{pay.invoice}</td>
                                            <td className="py-2 px-4 border text-center">{pay.date || "-"}</td>
                                            <td className="py-2 px-4 border text-center">{pay.description}</td>
                                            <td className="py-2 px-4 border text-center">Rp. {pay.amount.toLocaleString("id-ID")}</td>
                                            <td className="py-2 px-4 border text-center">{pay.method || "-"}</td>
                                            <td className={`py-2 px-4 border text-center font-bold ${pay.status === "Lunas" ? "text-green-600" : "text-red-600"}`}>
                                                {pay.status}
                                            </td>
                                            <td className="py-2 px-4 border text-center">{pay.year}</td>
                                            <td className="py-2 px-4 border text-center">{pay.keterangan}</td>
                                        </tr>
                                    ))
                                ) : (
                                    <tr>
                                        <td colSpan="9" className="py-4 text-center text-red-500">Data pembayaran tidak ditemukan</td>
                                    </tr>
                                )}
                            </tbody>
                        </table>
