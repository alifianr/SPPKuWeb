...
export default function DownloadPage() {
    const { payment } = usePage().props; // Perbaikan: Ambil `payment`, bukan `payments`
        ...
        <div className="bg-primary font-sans min-h-screen px-4 py-2 text-black">
            <div className="w-full mx-auto mt-6 rounded-lg px-16">
                ...
                <div className="text-center mb-6 text-black">
                    ...
                    <p>{payment.date || "-"} {payment.year || "-"}</p> {/* Pastikan data tidak null */}
                </div>
                ...
                <div className="mb-4">
                        ...
                        <table className="w-full border-collapse border border-gray-300 text-sm md:text-base">
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td className="border border-gray-300 p-2 text-center">{payment.invoice || "-"}</td>
                                    <td className="border border-gray-300 p-2 text-center">{payment.date || "-"}</td>
                                    <td className="border border-gray-300 p-2 text-center">{payment.description || "-"}</td>
                                    <td className="border border-gray-300 p-2 text-center">Rp. {payment.amount || "-"}</td>
                                    <td className="border border-gray-300 p-2 text-center">{payment.method || "-"}</td>
                                    <td
                                        className={`border border-gray-300 p-2 text-center ${payment.status === "Lunas" ? "text-green-600" : "text-red-600"
                                            }`}
                                    >
                                        {payment.status || "-"}
                                    </td>
                                    <td className="border border-gray-300 p-2 text-center">{payment.year || "-"}</td>
                    ...
                    {/* Tanda Tangan */}
                    <div className="text-right mb-14 mt-4">
                        <p>Bali, 28 April 2024</p>
                    </div>

                    {/* Tombol Kembali */}
                    <div className="text-center w-full bg-green-500 rounded-full py-2">
                        <Link href="/user/pembayaran-spp" className="w-full text-white px-6">
                            Download
                        </Link>
                    </div>
                </div>
